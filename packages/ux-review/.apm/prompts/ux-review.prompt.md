...
description: "AI ペルソナで Playwright MCP 経由の UX レビューを実施する"
---
...
1. **初回のみ**: `claude mcp add playwright -s user -- npx '@playwright/mcp@latest'` を実行して Claude Code を再起動
2. **プロジェクトごとのセットアップ**: `~/dotfiles/claude/ux-review/templates/` をプロジェクトの `evaluation/ux_review/` などにコピー
...
**詳しい使い方ドキュメント**: `~/dotfiles/claude/ux-review/Introduction.md`
...
```text
/ux-review blueprint/evaluation/ux_review newcomer
/ux-review blueprint/evaluation/ux_review ip_veteran free_exploration
```
...
新サービスに骨格を展開する。`~/dotfiles/claude/ux-review/templates/` の内容を
`<target-path>` にコピーする。
...
1. **Playwright MCP が接続されているか**
   ...
   - 未接続の場合はユーザーにこう案内して停止:
     > Playwright MCP が接続されていません。以下を実行してから Claude Code を再起動してください:
     >
     > ```bash
...
`cp -R ~/dotfiles/claude/ux-review/templates/. <target-path>/` しても同じ結果になるが、
このサブコマンドを使うと編集ポイントの案内までセットで実行される。

1. `<target-path>` が既存なら「既に存在します。上書きを避けるため中断します。」と伝えて停止
2. `mkdir -p <target-path>` でディレクトリ作成
3. `cp -R ~/dotfiles/claude/ux-review/templates/. <target-path>/` でコピー（この dotfiles ディレクトリが雛形の正本。更新はここを直接編集する）
4. コピー後、ユーザーに以下を **編集必須** として案内:
   - `<target-path>/service.md` — URL・ログイン手順・プレフィックス規約・レート制約
   - `<target-path>/tasks/<persona>.md` — そのサービス固有の典型タスク
   - `<target-path>/README.md` — プロジェクト固有の補足（任意。blueprint の `evaluation/ux_review/README.md` を参考に）
