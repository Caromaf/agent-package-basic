...
- `discover-signals` で `.triage/signals-<date>.json` が生成済み
- 編集対象ファイルが手元にある:
  - `~/dotfiles/claude/profiles/<pc>/CLAUDE.md`
  - 別リポ(例 `Caromaf/agent-package-basic`)の SKILL.md → ローカルに clone 済みのパスをユーザーが指定する

## 入力

```text
triage-improvements [--signals <path>] [--basic-repo <path>]
```

- `--signals <path>`: 入力 JSON。省略時は `.triage/signals-*.json` の最新を選択
- `--basic-repo <path>`: `agent-package-basic` のローカル clone パス。SKILL.md 追記が必要なときのみ指定。省略時は SKILL.md 系のシグナルはユーザーに「クローンパスを入力するか、見送るか」を尋ねる
...
| 内容 | 反映先 |
| --- | --- |
| 全般的な振る舞い・文体・確認スタイル | `~/dotfiles/claude/profiles/<pc>/CLAUDE.md` |
| 特定 skill の使い方の問題 | `<basic-repo>/packages/<name>/SKILL.md` |
| 判定不能 | ユーザーに選んでもらう |
...
```bash
rg -n "deprecated|存在確認" ~/dotfiles/claude/profiles/wsl-ubuntu/CLAUDE.md
```

重複している場合は新規追加ではなく **既存セクションの拡張** として diff を作る。

### Step 4: ユーザーへの提示 (バッチ)

採用候補を全件まとめて以下の形式で提示する:

```text
📋 triage-improvements: 採用候補 N 件

入力: .triage/signals-<date>.json (M 件中 N 件を採用候補に選定)

────────────────────────────────────────────────────
[sig-0001] user_correction → CLAUDE.md
sig 内容: 「apm.yml の package が悪さしている?」
採用理由: 設定起因と決めつけて実装を見ない癖を抑止
追記先: ~/dotfiles/claude/profiles/wsl-ubuntu/CLAUDE.md
追記内容:
  - **設定ファイルだけで原因を断定しない**。設定が無関係な可能性も考慮し、
    まず実装(コードや hook 出力)を確認する。
  - 理由: ユーザーが既に設定を点検済みのケースが多く、設定起因と推測すると
    時間の無駄になる。

(既存セクションへの統合: なし / 新規 "## 観察と推論" セクションへ追加)
────────────────────────────────────────────────────
[sig-0004] tool_loop → SKILL.md
sig 内容: extract_signals.py を 7 回 Edit
追記先: <basic-repo>/packages/discover-signals/SKILL.md  (※ --basic-repo 未指定)
追記内容: ...
...
編集したファイルを `git add` する。**commit はしない**。

```bash
# dotfiles 側
cd ~/dotfiles
git add claude/profiles/wsl-ubuntu/CLAUDE.md

# agent-package-basic 側 (--basic-repo が指定された場合のみ)
cd <basic-repo>
git add packages/<name>/SKILL.md
```

`--basic-repo` が指定されておらず SKILL.md 系の採用候補があった場合は、**対象ファイルのフルパスと提案文面を `.triage/pending-skill-md-<date>.md` に書き出す** だけにとどめる。ユーザーが後でクローン取得後に手動で適用できるようにする。
...
入力: .triage/signals-<date>.json (M 件)
採用: N 件 / 見送り: K 件

ファイル変更 (staged):
  ~/dotfiles/claude/profiles/wsl-ubuntu/CLAUDE.md  (+12 行)

未適用 (--basic-repo 未指定):
  packages/discover-signals/SKILL.md  → .triage/pending-skill-md-2026-05-29.md に保存

次のステップ:
- 差分確認: cd ~/dotfiles && git diff --cached
...
`~/dotfiles/claude/profiles/<pc>/CLAUDE.md` にある「Markdown 文章スタイル」セクションを順守:
