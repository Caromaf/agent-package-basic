...
description: GitHub Issue に 1 つ以上の Sub-issue を紐付ける必要があるときに使用する。クロスリポジトリ対応で、紐付け後は親 Issue と全子 Issue を GitHub Project に追加し Status / Priority / Start date を設定する。`create-issue` と `~/.claude/custom-config/create-issue-config.json` (または `~/.codex/custom-config/create-issue-config.json`) を共有して二重管理を防ぐ。
---
...
GitHub Project への追加に使用する設定は `create-issue-config.json` から読み込みます。
`create-issue` skill と設定を共有し、二重管理を防ぎます。

設定ファイルは以下の優先順で探索します（先に見つかったものを使用）:

1. `~/.claude/custom-config/create-issue-config.json`
2. `~/.codex/custom-config/create-issue-config.json`

```bash
CONFIG_FILE=""
if [ -f "$HOME/.claude/custom-config/create-issue-config.json" ]; then
  CONFIG_FILE="$HOME/.claude/custom-config/create-issue-config.json"
elif [ -f "$HOME/.codex/custom-config/create-issue-config.json" ]; then
  CONFIG_FILE="$HOME/.codex/custom-config/create-issue-config.json"
fi
```
...
設定ファイル (`$CONFIG_FILE`) から `project.owner` と `project.number` を読み込み、
親 Issue と全子 Issue を GitHub Project に追加します。

```bash
# 設定ファイルから Project 情報を読み込み
PROJECT_OWNER=$(jq -r '.project.owner' "$CONFIG_FILE")
